        ...
        try {
            ...
            for (const item of items) {
                if (item.olive_category_id) {
                    const model = (item.model_type === 'Product' ? Product : OliveCategory) as any;
            ...
            try {
                const owners = await User.find({ role: 'owner' }, '_id');
                const orderRef = (order._id as any).toString().slice(-6).toUpperCase();
...
router.patch('/:id/notes', authenticate, ownerOnly, async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        const order = await Order.findByIdAndUpdate(req.params.id, { owner_notes: req.body.notes }, { new: true });
        if (!order) {
            res.status(404).json({ message: 'Commande introuvable.' });
            return;
        }
